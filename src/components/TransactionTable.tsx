...
const TransactionTable = ({ limit = 7 }: ITransactionsTable) => {
  ...
  return (
    <>
      ...
      {totalPages > 1 && (
        <ReactPaginate
          className="mt-3 sm:mt-4 flex items-center justify-center sm:justify-end gap-1 sm:gap-2"
      ...
      {/* Таблица */}
      <div className="mt-3 sm:mt-4 overflow-hidden rounded-xl sm:rounded-2xl border border-slate-700/50 bg-slate-800/50 shadow-2xl shadow-black/20 backdrop-blur-xl">
                  ...
                  >
                    ...
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        ></div>
                        <span className="font-medium text-white truncate">{transaction.title}</span>
                      </div>
                      <span className="inline-flex items-center rounded-full border border-slate-600/50 bg-slate-700/50 px-2 py-0.5 text-xs text-gray-300 flex-shrink-0">
          ...
          <table className="hidden w-full sm:table">
            <thead>
              <tr className="border-b border-slate-700/50">
                <td className="p-3 sm:p-4 text-xs sm:text-sm font-semibold tracking-wider text-gray-300 uppercase">#</td>
                <td className="p-3 sm:p-4 text-xs sm:text-sm font-semibold tracking-wider text-gray-300 uppercase">Title</td>
                <td className="p-3 sm:p-4 text-xs sm:text-sm font-semibold tracking-wider text-gray-300 uppercase">Amount</td>
                <td className="p-3 sm:p-4 text-xs sm:text-sm font-semibold tracking-wider text-gray-300 uppercase">Category</td>
                <td className="p-3 sm:p-4 text-xs sm:text-sm font-semibold tracking-wider text-gray-300 uppercase">Date</td>
                <td className="p-3 sm:p-4 text-center text-xs sm:text-sm font-semibold tracking-wider text-gray-300 uppercase">Action</td>
              </tr>
            </thead>
            <tbody>
              {data?.length > 0 ? (
                data.map((transaction, index) => (
                  ...
                  >
                    <td className="p-3 sm:p-4 text-sm font-medium text-gray-300">{(currentPage - 1) * limit + index + 1}</td>
                    <td className="p-3 sm:p-4">
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        ></div>
                        <span className="text-sm sm:text-base font-medium text-white">{transaction.title}</span>
                      </div>
                    </td>
                    <td className="p-3 sm:p-4">
                      <div
                        className={`inline-flex items-center gap-1 rounded-full px-2 sm:px-3 py-1 text-xs sm:text-sm font-semibold ${
                          transaction.type === "income"
                            ? "border border-emerald-500/30 bg-emerald-500/20 text-emerald-300"
                            : "border border-red-500/30 bg-red-500/20 text-red-300"
                        }`}
                    ...
                    <td className="p-3 sm:p-4">
                      <span className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-slate-600/50 bg-slate-700/50 px-2 sm:px-3 py-1 text-xs sm:text-sm text-gray-300">
                        {transaction.category?.title || "Other"}
                      </span>
                    </td>
                    <td className="p-3 sm:p-4 text-xs sm:text-sm text-gray-400">
                      {new Date(transaction.createdAt).toLocaleDateString("en-US", {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                    ...
                    <td className="p-3 sm:p-4">
                      <div className="flex justify-center">
                        <Form method="DELETE" action="/transactions">
                          <input type="hidden" name="id" value={transaction.id} />
                          <button
                            className="group/delete transform rounded-lg sm:rounded-xl border border-red-500/20 bg-red-500/10 p-1.5 sm:p-2 text-red-300 transition-all duration-200 hover:scale-110 hover:border-red-500/30 hover:bg-red-500/20 hover:text-red-200"
                            title="Delete transaction"
                          >
                            <FaTrash className="h-2.5 w-2.5 sm:h-3 sm:w-3 transition-transform duration-200 group-hover/delete:scale-110" />
                          </button>
                        </Form>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="p-6 sm:p-8 text-center">
                    <div className="flex flex-col items-center justify-center gap-3 text-gray-400">
                      <div className="flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center rounded-xl sm:rounded-2xl bg-slate-700/50">
                        <svg className="h-6 w-6 sm:h-8 sm:w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                        </svg>
                      </div>
                      <h3 className="text-base sm:text-lg font-semibold">No transactions found</h3>
      ...
      {data?.length > 0 && (
        <div className="mt-3 sm:mt-4 flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0 text-xs sm:text-sm text-gray-400">
          <div>
